...
| **That *asking* for a confidence is a mild reasoning step that helps classification** | **Measured FALSE, and the opposite is true — but it is not free.** Removing `type_confidence` and `considered_types` from the output schema **improved** every error metric (type accuracy 97.1% → 97.7%, `Event → Fact` 4 → 1) and **cost 3.2 pp of coverage** (99.1% → 95.9%), with entities per subject 0.99 → 0.96 — intermittent under-emission, no subject lost outright, no degenerate draws. So the ask is not a reasoning aid but does appear to be a *thoroughness* aid, by an unidentified mechanism. Both fields therefore stay in the prompt and are discarded rather than stored (§3.4). One observation per condition, no variance estimate — see below. *(#35)* |
