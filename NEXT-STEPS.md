...
**The fence (#36), `qwen3:14b`, three independent runs per condition:** `Event → Fact` **4.7 [3–6] → 1.0**, ranges non-overlapping; type accuracy **96.8% → 97.7%**; coverage **98.4% → 97.5%**. ⚠ **Do not cite the `qwen3:4b` fence figures** (`Event → Fact` 9.3 → 2.0, accuracy 91.1% → 92.6%). They are pre-upgrade, and 4b was never re-run on Ollama 0.32.4 — deliberately, since 4b lost the ladder and 14b ships — so every 4b number in this project is now a **cross-runtime** claim with no same-runtime baseline. Decided 2026-07-27: strike them rather than re-measure.
